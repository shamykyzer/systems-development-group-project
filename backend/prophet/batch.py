"""Standalone Prophet batch script for PNG output.

Run via Docker: docker compose -f docker/docker-compose.yml --profile forecasting up prophet
Or: python -m prophet.batch (from backend/ or with backend on PYTHONPATH)
"""
from prophet._fb import Prophet  # Facebook Prophet library (avoids self-shadowing)
import pandas as pd
import matplotlib.pyplot as plt
import os

# Resolve paths relative to backend root (where CSV_Files lives)
# When run from Docker: cwd is /app (backend root)
# When run locally: cwd may be backend/ or project root
_script_dir = os.path.dirname(os.path.abspath(__file__))
_backend_root = os.path.dirname(_script_dir)
_csv_dir = os.path.join(_backend_root, "CSV_Files")

# Load the CSV data
csv_path = os.path.join(_csv_dir, "Pink CoffeeSales March - Oct 2025.csv")
